...
export async function saveSearch(userEmailKey, query, database) {
  try {
    ...
    const purchaseSearchesRef = ref(database, `users/${userEmailKey}/recentSearches/purchase/${query}`);
    // Guardar solo el timestamp directamente
    await set(purchaseSearchesRef, timestamp);
  } catch (error) {
  ...
  recentSearchesContainer.addEventListener("click", async (e) => {
    ...
    if (deleteBtn && recentSearchesContainer.contains(deleteBtn)) {
      ...
      showConfirmModal(
        ...
        async () => {
          try {
            const searchRef = ref(_database, `users/${userEmailKey}/recentSearches/purchase/${query}`);
...
export async function displayRecentSearches(userEmailKey, database) {
  try {
    const purchaseSearchesRef = ref(database, `users/${userEmailKey}/recentSearches/purchase`);
    const snapshot = await get(purchaseSearchesRef);

    const recentSearchesContainer = document.getElementById("recentSearches");
    if (!recentSearchesContainer) return;

    if (snapshot.exists()) {
      const purchaseSearches = snapshot.val();

      // Convertir en array de entradas [query, timestamp] y ordenar por timestamp descendente
      const sortedSearches = Object.entries(purchaseSearches)
