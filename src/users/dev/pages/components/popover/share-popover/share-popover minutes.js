...
// Función para calcular la fecha de expiración en minutos
function calculateExpiration(durationInMinutes) {
...
document.addEventListener('click', async (event) => {
  if (event.target && event.target.id === 'sendEmailButton') {
    ...
    const durationInMinutes = parseInt(selectedDuration.value, 10); // Convertir a número
    const expirationDate = calculateExpiration(durationInMinutes);
